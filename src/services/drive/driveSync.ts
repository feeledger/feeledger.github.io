...
export async function pushAllToDrive(token: string): Promise<void> {
  ...
  const [
    ...
    db.receipts.toArray(),
    db.studentFields.toArray().then(rows => rows.map(({ _store: _, ...f }) => f as StudentFieldDefinition)),
    db.paymentFields.toArray().then(rows => rows.map(({ _store: _, ...f }) => f as PaymentFieldDefinition)),
